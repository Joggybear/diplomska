...
then echo "$ERR Was unable to locate file '${DIRECTORY}/${INDEXFILE}'."
     exit 1
fi
...
then echo "$ERR AWS-CLI is not installed."
	 echo "$INF To install AWS-CLI, visit:"
	 echo "     http://docs.aws.amazon.com/cli/latest/userguide/installing.html"
	 exit 1
fi

# CHECK BUCKET
BUCKET_EXISTS=`aws s3 ls s3://${BUCKET} 2>&1 --profile ${PROFILE} | grep -e "NoSuchBucket" -e "AllAccessDisabled"`
if   [ -n "$BUCKET_EXISTS" ]
...
# DEPLOY APP
REGION=`aws s3api get-bucket-location --bucket ${BUCKET%%/*} --output text --profile ${PROFILE}`
if [ "$REGION" == "None" ]
...
aws s3 website s3://${BUCKET%%/*} --index-document ${INDEXFILE##*/} --error-document ${INDEXFILE##*/} --region ${REGION} --profile ${PROFILE}
